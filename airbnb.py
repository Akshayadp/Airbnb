...
if selected == "Home":
    ...
    st.write("Airbnb is an American company operating an online marketplace for short- and long-term homestays and experiences. The company acts as a broker and charges a commission from each booking. The company was founded in 2008 by Brian Chesky, Nathan Blecharczyk, and Joe Gebbia. Airbnb is a shortened version of its original name, AirBedandBreakfast.com. Airbnb is the most well-known company for short-term housing rentals.")  
    #st.header("Background of Airbnb")
    #st.write("...")  
    st.header("About the Project")
    st.write("This project involves the MongoDB Atlas sample Airbnb dataset. It's objective is to conduct Exploratory Data Analysis (EDA) by performing data cleaning. Ultimately, we aim to represent the insightful findings in a visually compelling format through storytelling using Power BI.")
    
# Data Exploration page
if selected == "Data Analysis":
    #st.title("Data Exploration")
    with st.sidebar:
        select = option_menu("ANALYSIS & VISUALIZATION", ["PRICE", "AVAILABILITY", "LOCATION", "GEOSPATIAL VISUALIZATION", "TOP ANALYSIS"])
    ...
    if select == "PRICE":
        ...
        with col1:
            fig_bar= px.bar(df_bar, x='property_type', y= "price", title= "PRICE DIFFERENCE BASED ON PROPERTY TYPES",
        ...
        with col2:
            ...
            title='BEDROOMS AND BEDS ACCOMMODATIONS',hover_data="price",
    ...
    if select == "AVAILABILITY":
        ...
        with col1:
            ...
            grouped_df = df_a.groupby(["room_type", "bed_type", "longitude", "latitude"]).sum()
        ...
        with col1:
            ...
            fig_a_sunb_30 = px.sunburst(grouped_df, path=["room_type", "bed_type", "is_location_exact"],
                                        values="availability_30", title="Availability_30",
                                        color_discrete_sequence=px.colors.sequential.Aggrnyl_r)

            st.plotly_chart(fig_a_sunb_30)

        with col2:
            fig_a_sunb_60 = px.sunburst(grouped_df, path=["room_type", "bed_type", "is_location_exact"],
                                        values="availability_60", title="Availability_60",
                                        color_discrete_sequence=px.colors.sequential.Blues_r)

            st.plotly_chart(fig_a_sunb_60)

        col1, col2 = st.columns(2)

        with col1:
            fig_a_sunb_90 = px.sunburst(grouped_df, path=["room_type", "bed_type", "is_location_exact"],
                                        values="availability_90", title="Availability_90",
                                        color_discrete_sequence=px.colors.sequential.Aggrnyl_r)

            st.plotly_chart(fig_a_sunb_90)

        with col2:
            fig_a_sunb_365 = px.sunburst(grouped_df, path=["room_type", "bed_type", "is_location_exact"],
                                        values="availability_365", title="Availability_365",
                                        color_discrete_sequence=px.colors.sequential.Greens_r)

            st.plotly_chart(fig_a_sunb_365)
    ...
    if select == "LOCATION":
        ...
        st.title("**LOCATION WISE ANALYSIS**")
